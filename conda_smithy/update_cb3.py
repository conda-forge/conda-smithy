...
class MyDoubleQuotedScalarString(
    ruamel.yaml.scalarstring.DoubleQuotedScalarString
):
    __slots__ = "lc"


class MySingleQuotedScalarString(
...
def get_compilers(url):
    ...
    with tmp_directory() as tmp_dir:
        ...
        with tarzip_open(os.path.join(tmp_dir, fname)) as tf:
            need_f = any(
                [
                    name(f).lower().endswith((".f", ".f90", ".f77"))
                    for f in iterate(tf)
                ]
            )
            ...
            need_cxx = any(
                [
                    name(f).lower().endswith((".cxx", ".cpp", ".cc", ".c++"))
                    for f in iterate(tf)
                ]
            )
            for f in iterate(tf):
                if name(f).lower().endswith("setup.py"):
                    try:
                        ...
                        if (
                            "numpy.get_include()" in content
                            or "np.get_include()" in content
                        ):
                            need_numpy_pin = True
                    except:
                        pass
...
def update_cb3(recipe_path, conda_build_config_path):
    ...
    change_lines[reqbuild_s - 1] = (
        reqbuild_line,
        reqbuild_line.replace("build:", "host:"),
    )
    ...
    for i in range(requirements_section.start, requirements_section.end + 1):
        ...
        if line.startswith("- "):
            ...
            req_rendered = (
                content2[i].strip()[2:].strip().split(" ")[0].strip()
            )
            ...
            if req_rendered in [
                ...
                messages[
                    "Removing {} in favour of compiler()".format(req)
                ] = True
                change_lines[i] = (lines[i], None)
                need_c = True
                if req in ["m2w64-toolchain", "mingwpy"] or (
                    req != req_rendered
                    and req_rendered in ["toolchain", "toolchain3"]
                ):
                    need_mingw_c = True
                continue
            ...
            if req == "python" and "# [win]" in line:
                messages[
                    "Moving `python # [win]` which was used for vc matrix".format(
                        req
                    )
                ] = True
                change_lines[i] = (lines[i], None)
            ...
            if req.replace("-", "_") in pinned_packages or (
                ...
                if (
                    len(s) > 2
                    and not s[2].startswith("#")
                    and i not in change_lines
                ):
                    if (
                        not req.replace("-", "_") in pinned_packages
                        and not ("m2w64-" + req_rendered.replace("-", "_"))
                        in pinned_packages
                        and (
                            "# [not win]" not in line
                            and "# [unix]" not in line
                        )
                    ):
                        msg = "Not sure how to remove pinnings for {}".format(
                            req
                        )
                    else:
                        change_lines[i] = (
                            lines[i],
                            lines[i].replace(s[2], " " * len(s[2])),
                        )
                        msg = (
                            "Removing pinnings for {} to use values from "
                            "conda_build_config.yaml. If you need the pin see "
                            "[here](https://conda-forge.readthedocs.io/en/latest/docs/meta.html#pinning-packages) "
                    ...
                    if req == "numpy":
                        ...
                        if need_numpy_pin and i > reqbuild_section.end:
                            ...
                            msg = (
                                ...
                                "[here](https://conda-forge.readthedocs.io/en/latest/docs/meta.html"
    ...
    skip_lines = [
        ...
        if i >= build_section.start
        and i <= build_section.end
        and line.strip().startswith("skip:")
    ]
    ...
    if python_win_matrix and not python_dep:
        for i, line in skip_lines:
            ...
            if len(skip_lines) == 1 and skip_line in [
                ...
                messages[
                    "Removed skip for py2k and added skip for vc<14"
                ] = True
                change_lines[i] = (
                    line,
                    line[: line.find("#")] + "# [win and vc<14]",
                )
    ...
    for i, line in enumerate(lines):
        ...
        if line.strip().startswith(vc14):
            ...
            change_lines[i] = (
                line,
                line.replace(vc14, "skip: True  # [win and vc<14]"),
            )
    ...
    if features_section is not None:
        ...
        if remove_features_section:
            ...
            change_lines[features_section.start - 1] = (
                lines[features_section.start - 1],
                None,
            )
    ...
    def add_compiler(name, p_name):
        if need_mingw_c:
            build_lines.append(
                build_space + "{{ compiler('" + name + "') }}        # [unix]"
            )
            build_lines.append(
                build_space + "{{ compiler('m2w64_" + name + "') }}  # [win]"
            )
            messages[
                "Adding " + p_name + " compiler with mingw for windows"
            ] = True
        else:
    ...
    if build_lines:
        build_lines = [
            " " * (len(reqbuild_line) - len(reqbuild_line.lstrip())) + "build:"
        ] + build_lines
    ...
    if is_r_package:
        ...
        change_lines[pos] = (
            lines[pos],
            lines[pos]
            + " " * (len(lines[pos + 1]) - len(lines[pos + 1].lstrip()))
            ...
            if ind != -1:
                ...
                for x in [
                    ...
                    if x in select:
                        ...
                        messages[
                            "Changed {} in selector {} to vc<14".format(
                                x, select
                            )
                        ] = True
                for x in [
